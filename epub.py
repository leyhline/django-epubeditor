...
def build_smil(groups: list[list[SpanData]], text_src: str, audio_src: str) -> SmilTree:
    namespaces = SmilTree.register_namespaces()
    smil = Element("smil", {"xmlns": namespaces[""], "xmlns:epub": namespaces["epub"], "version": "3.0"})
    body = SubElement(smil, "body")
    seq = SubElement(body, "seq", {"id": "seq1", "epub:textref": text_src, "epub:type": "bodymatter chapter"})
    index = 1
    for paragraph in groups:
        for data in paragraph:
            par = build_smil_par(data, text_src, audio_src, index)
            seq.append(par)
            index += 1
    indent(smil, "  ")
    return SmilTree(smil)
